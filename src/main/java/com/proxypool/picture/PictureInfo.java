...
public class PictureInfo extends BaseEntityInfo {
    ...
    public void setWidthAndHeight(String content) {
        ...
        List<String> widthHeightInfo = Lists.newArrayList();
        Iterable<String> it = Splitter.on("x").split(content);
        it.forEach(item -> {
            widthHeightInfo.add(item);
        });
        String[] infoAr = (String[]) widthHeightInfo.toArray();
